    ...
    formatDeltaChildren(
        ...
        this.forEachDeltaKey(delta, left, (key, leftKey, movedFrom, isLast) => {
            this.recurse(
                ...
                left
                    ? (left as Record<string | number, unknown>)[leftKey]
                    : undefined,
                key,
                leftKey,
                movedFrom,
    ...
    forEachDeltaKey(
        ...
        if (typeof left !== 'undefined') {
            for (name in left) {
                if (Object.prototype.hasOwnProperty.call(left, name)) {
                    if (
                        typeof (delta as Record<string, Delta>)[name] ===
                            'undefined' &&
                        (!arrayKeys ||
                            typeof (delta as ArrayDelta)[
                                `_${name}` as `_${number}`
                            ] === 'undefined')
                    ) {
                        keys.push(name);
                ...
                if (Array.isArray(value) && value[2] === 3) {
                    ...
                    moveDestinations[`${movedDelta[1]}`] = {
                        ...
                        value:
                            left &&
                            (left as unknown[])[
                                Number.parseInt(name.substring(1), 10)
                            ],
                    };
                    if (this.includeMoveDestinations !== false) {
                        if (
                            typeof left === 'undefined' &&
                            typeof (delta as ArrayDelta)[movedDelta[1]] ===
                                'undefined'
                        ) {
        ...
        for (let index = 0, length = keys.length; index < length; index++) {
            ...
            const leftKey = arrayKeys
                ? Number.parseInt(trimUnderscore(key), 10)
                : key;
